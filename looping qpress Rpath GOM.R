...
#create list
mod<-list(GOMqnm10,GOMqnm20, GOMqnm30, GOMqnm40, GOMqnm50)
...
for(i in seq_along(q.models)){
  ...
  write.dia(q.models[[i]], paste0("GOM", i,".dia"))
}

#loop through models to create results csv file and pdf barplots
  #create list of scenarios
# scen<-list('Phytoplankton', 'Mesozoop', 'Megabenthos','Small pelagics', 'D piscivores', 'Seals', 'Fishery')
# neg_scen<-list('Macrobenthos', 'Macrozoop', 'D piscivores', 'Fishery')
# scenarios<-vector('list', length(scen))  
...
for(j in seq_along(q.models)){
  ...
  while(i < n.sims) {
    ...
    write.csv(results, paste0("GOM","results", j,"D piscivores_down",".csv"))
    ...
    filename= paste("GOM", "plots", j,"D piscivores_down", ".pdf")
    pdf(filename)
